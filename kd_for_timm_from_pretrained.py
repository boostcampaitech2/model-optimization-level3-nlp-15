...
def train(
    ...
    ################################################
    student_model_instance = Model(student_model_config, verbose=True)
    student_path = "/opt/ml/code/exp/final/best.pt"
    ...
    # student_model_instance.load_state_dict(student_state_dict)
    ################################################

    ####Change Model Name for TIMM Models ####
    teacher_model = timm.create_model(
        "swsl_resnext50_32x4d", pretrained=False, num_classes=6
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--student_model",
        default="/opt/ml/code/exp/final/model.yml",
        type=str,
        help="student model config",
    )
    parser.add_argument(
        "--weight",
        default="/opt/ml/code/exp/teacher/best.pt",
        type=str,
    ...
    log_dir = os.environ.get("SM_MODEL_DIR", os.path.join("exp", "student"))
